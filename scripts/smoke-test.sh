...
echo -e "\nCreate - POST $URL\n"
curl -s --show-error -X POST $URL -H "Content-Type: application/json" -d '{"title":"Test Content","body":"This is test content","author":"Test Author","status":"draft"}' -o $OUTPUT_DIR/create.json
cat $OUTPUT_DIR/create.json | jq

URL="$BASE_URL/content"
echo -e "\nList - GET $URL\n"
curl -s --show-error $URL -o $OUTPUT_DIR/list.json
...
echo -e "\nGet - GET $URL\n"
curl -s --show-error $URL -o $OUTPUT_DIR/get.json
...
curl -s --show-error -X PUT $URL \
  ...
  -d '{
    ...
    "status": "published",
    "data": {"foo": "bar"}
  }' -o $OUTPUT_DIR/update.json
cat $OUTPUT_DIR/update.json | jq

URL="$BASE_URL/content/$CONTENT_ID"
echo -e "\nDelete - DELETE $URL\n"
curl -s --show-error -X DELETE $URL -o $OUTPUT_DIR/delete.json
